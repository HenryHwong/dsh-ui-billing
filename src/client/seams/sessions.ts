 ...
 */
export function currentSessionProjection(uiSession: UiSession): CurrentSessionProjection {
  const current = uiSession.adapter.current
  return {
    subscribe: listener => current.subscribe(listener),
    face: key => current.getSnapshot().keyedHooks.projection?.(key) as HostObservable<unknown> | undefined,
  }
}
